...
void to_little_endian(const mbedtls_mpi &value, std::span<std::byte> out)
{
                                 ...
                                 big_endian.size()) != 0)
    {
        throw std::runtime_error(
            "adbcpp: failed to serialize an RSA component");
    }
...
Key Key::load_or_generate()
{
    ...
    if (std::filesystem::exists(private_path))
    {
        ...
        mbedtls_entropy_init(&entropy);
        mbedtls_ctr_drbg_init(&drbg);
        const int seed = mbedtls_ctr_drbg_seed(&drbg, mbedtls_entropy_func,
                                               &entropy, nullptr, 0);
        ...
        {
            throw std::runtime_error(
                "adbcpp: failed to load the ADB private key (" +
                std::to_string(rc) + ")");
        }
