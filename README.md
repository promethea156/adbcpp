...
> **Status:** early development. Slice 0 (the walking skeleton), Slice 1 (shell over USB), Slice 2 (file listing over `sync`), Slice 3 (pull over `sync`), Slice 4 (push, stat over `sync`), and Slice 5 (install, uninstall) are complete, and the [error model](docs/07-error-model.md) is in place: every fallible operation returns a `Result<T>` instead of throwing. See [`docs/03-roadmap.md`](docs/03-roadmap.md) for the plan.
