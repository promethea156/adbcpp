...
# depends on it. Set -DADBCPP_CLANG_FORMAT=<path> to use a specific binary.
#
...
find_program(ADBCPP_CLANG_FORMAT
...
if(ADBCPP_CLANG_FORMAT)
    file(GLOB_RECURSE ADBCPP_FORMAT_SOURCES
        CONFIGURE_DEPENDS
    ...
    add_custom_target(format
        COMMAND "${ADBCPP_CLANG_FORMAT}" -i ${ADBCPP_FORMAT_SOURCES}
        COMMENT "Formatting adbcpp sources with clang-format"
        VERBATIM
    )

    add_custom_target(format-check
        COMMAND "${ADBCPP_CLANG_FORMAT}" --dry-run --Werror
            ${ADBCPP_FORMAT_SOURCES}
        COMMENT "Checking adbcpp formatting with clang-format"
        VERBATIM
    )
else()
    message(STATUS
        "clang-format not found; the 'format' and 'format-check' targets are "
        "disabled. Set -DADBCPP_CLANG_FORMAT=<path> to enable them.")
endif()
