          ...
          "[connection]")
{
    ...
    const auto cnxn =
        make_message(adbcpp::protocol::kCnxn, adbcpp::protocol::kVersion,
                     adbcpp::protocol::kMaxData, identity);
    const auto auth_out = make_message(
        adbcpp::protocol::kAuth, adbcpp::protocol::kAuthPublicKey, 0, key);
          ...
          "[connection]")
{
    ...
    const adbcpp::Connection::Signer signer =
        [signature](std::span<const std::byte>)
    {
        return signature;
    };
    ...
    const auto cnxn =
        make_message(adbcpp::protocol::kCnxn, adbcpp::protocol::kVersion,
                     adbcpp::protocol::kMaxData, identity);
    const auto signed_auth =
        make_message(adbcpp::protocol::kAuth, adbcpp::protocol::kAuthSignature,
                     0, signature);
    const auto public_auth = make_message(
        adbcpp::protocol::kAuth, adbcpp::protocol::kAuthPublicKey, 0, key);
