...
- **Resolution**: `kSystemIdentity` now names only `shell_v2,stat_v2,ls_v2`, `delayed_ack` is still appended when it is enabled, and `sendrecv_v2,sendrecv_v2_zstd` is appended only when the zstd codec is built in, so the banner never claims a service the build cannot honor (issue #1).
- **Note**: The device test exercises `run`, `list`, `pull`, `push`, `install`, `uninstall`, `launch`, `is_running`, and `close` against the banner, so every feature the services rely on is kept. The lz4 and brotli variants would be a future improvement, not a claim (issues #35 and #34).
