...
- **Cause**: The USB transport applies a timeout to each bulk transfer, which carries at most one message header or payload. It was a fixed 120 seconds, and on one run the device stopped moving a single chunk for longer than that. A file transfer is a long sequence of small transfers and is never given one deadline for the whole file, but a long enough stall in one chunk still fails it. Throughput is otherwise at parity with `adb` (about 8 MB/s each way on the test device), so this is a slow-device tolerance problem rather than a protocol defect.
- **Resolution**: The timeout is now configurable, as `UsbTransport(id, transfer_timeout_ms)` or `set_transfer_timeout`, with `kDefaultTransferTimeoutMs` as the default, and the USB example exposes it as `--timeout <ms>`. Separately, `push` now writes the sync `DATA` header and its chunk in one message instead of two, like adb's `WriteOrDie`, which halves the messages and the acknowledgements per chunk.
- **Note**: With both changes, 64, 128, and 256 MiB round-trip reliably at the default timeout, verified over repeated runs. A device or host that is slower still should raise the timeout rather than expect a fixed value to fit every machine.
