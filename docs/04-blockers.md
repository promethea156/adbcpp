...
- **Resolution**: `kSystemIdentity` now names only `shell_v2,stat_v2,ls_v2`, `delayed_ack` is still appended when it is enabled, and `sendrecv_v2` and each built codec (`sendrecv_v2_zstd`, `sendrecv_v2_lz4`) are appended only when they are built in, so the banner never claims a service the build cannot honor (issues #1 and #35).
- **Note**: The device test exercises `run`, `list`, `pull`, `push`, `install`, `uninstall`, `launch`, `is_running`, and `close` against the banner, so every feature the services rely on is kept. The brotli variant would be a future improvement, not a claim (issue #34).
