...
| 3 | Use `sendrecv_v2` with brotli ([#34](https://github.com/promethea156/adbcpp/issues/34)) for `pull` and `push` | P6. The zstd and lz4 forms are done; brotli remains. |
...
- **Use `sendrecv_v2` with zstd for `pull` and `push`.** **Done.** The v1 `RECV`/`SEND` forms were always sent, so a transfer was never compressed. `pull` and `push` now send the v2 forms with zstd when the device advertised `sendrecv_v2_zstd` and the build has `ADBCPP_BUILD_COMPRESSION`, and the v1 forms otherwise, so a device without the feature still works. The banner advertises `sendrecv_v2` and only the built codecs. ([issue #1](https://github.com/promethea156/adbcpp/issues/1), [#35](https://github.com/promethea156/adbcpp/issues/35)); brotli ([#34](https://github.com/promethea156/adbcpp/issues/34)) remains.
