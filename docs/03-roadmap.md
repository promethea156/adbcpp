...
| 3 | Use `sendrecv_v2` with lz4 ([#35](https://github.com/promethea156/adbcpp/issues/35)) and brotli ([#34](https://github.com/promethea156/adbcpp/issues/34)) for `pull` and `push` | P6. The zstd form is done; these are the other codecs. |
...
- **Use `sendrecv_v2` with zstd for `pull` and `push`.** **Done.** The v1 `RECV`/`SEND` forms were always sent, so a transfer was never compressed. `pull` and `push` now send the v2 forms with zstd when the device advertised `sendrecv_v2_zstd` and the build has `ADBCPP_BUILD_COMPRESSION`, and the v1 forms otherwise, so a device without the feature still works. The banner advertises `sendrecv_v2,sendrecv_v2_zstd` only when the codec is built in. ([issue #1](https://github.com/promethea156/adbcpp/issues/1)); lz4 ([#35](https://github.com/promethea156/adbcpp/issues/35)) and brotli ([#34](https://github.com/promethea156/adbcpp/issues/34)) remain.
