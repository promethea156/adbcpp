...
enum class LogLevel
{
    ...
    Warning,
    /// A state change: a connection, a stream, or an authorization.
    Info,
...
/**
 * @brief Installs `sink` as the process-wide log sink and writes every level from
 * `Error` up to and including `level`.
 *
 ...
 */
ADBCPP_API void set_logger(LogSink sink, LogLevel level = LogLevel::Info);
 ...
 */
ADBCPP_API void log(LogLevel level, std::string_view message);
