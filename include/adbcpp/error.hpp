...
{
    /// The caller passed something the library cannot use: a path past the sync
    /// limit, a local file that is not a regular file, or a write past the
    /// negotiated maximum payload.
    InvalidArgument,
    ...
    Protocol,
    /// The device refused the request and gave a reason, which is the message. This
    /// is a sync `FAIL`.
    Device,
